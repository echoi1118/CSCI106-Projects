...
void DateType::ReadDate()
{
	...
	cout << setw(32) << "  The month is:";
	cin >> month;
	cout << setw(32) << "  The day is:";
	cin >> day;
	cout << setw(32) << "  The year is:";
	cin >> year;
	...
	while (IsValidDate(month, day, year) == false)
	{
		...
		cout << " Error  ||  Invalid Date  ||  Re-entry Required" << endl;
		SetConsoleTextAttribute(hConsole, 7);
		cout << setw(32) << "  You entered:" << month << "/" << day << "/" << year << endl << endl;
...
bool DateType::operator> (const DateType & aDate)
{
	...
	if (ComparedTo(aDate) == GREATER)		// Got the idea to use ComparedTo() instead of recreating all of the
		return true;						// comparisons from the professor's solutions.
	return false;							// Brilliant. Why didn't I see this before the turn-in?
}
...
int DateType::operator - (const DateType & aDate)
{
	...
	// So if (temp == d2) from the get-go, we never go into the loop and... (cont. on line 549)
	while (temp != d2)
	{
		if (temp < d2)
		{
	...
	return i;						// (cont. from line 535) ...we return 0, meaning temp == d2.
}
